...
// Get all orders using database.php
$orders = $db->getAllOrdersForReport();

// Enrich orders with payment info and apply filter
$filtered_orders = [];

foreach ($orders as &$order) {
    $payment = $db->getTotalPaidByOrderId($order['order_id']);

    $order['total_paid'] = $payment['total_paid'] ?? 0;
    $order['payment_status'] = ($order['total_paid'] >= $order['total_amount']) ? 'Paid' : 'Unpaid';

    if (isset($_GET['filter']) && $_GET['filter'] !== '') {
        if ($_GET['filter'] === $order['payment_status']) {
  ...
  <div class="row">
    <?php if (count($filtered_orders) > 0): ?>
      <?php foreach ($filtered_orders as $order): ?>
        <?php $items = $db->getOrderItemsByOrderId($order['order_id']); ?>
